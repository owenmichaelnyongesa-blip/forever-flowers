// ============================================================
//  FOREVER FLOWERS — MAIN SCRIPT
//  ✏️  CLIENT: You do NOT need to edit this file.
//      Edit products.js to change products & testimonials.
// ============================================================

const WA_NUMBER = "254790022080"; // ← WhatsApp number (no + or spaces)
...
function getBadgeClass(badge) {
  const map = { Sale: "badge-sale", New: "badge-new", Popular: "badge-popular" };
  return map[badge] || "";
}
...
function renderProducts(list) {
  ...
  list.forEach(p => {
    const badge = p.badge
      ? `<div class="product-badge ${getBadgeClass(p.badge)}">${p.badge}</div>` : "";
    const origPrice = p.originalPrice
      ? `<span class="price-original">${formatPrice(p.originalPrice)}</span>` : "";

    // Rich automated message — client sees exactly what product & price customer wants
    const priceInfo = p.originalPrice
      ? `${formatPrice(p.price)} (was ${formatPrice(p.originalPrice)})`
      : formatPrice(p.price);

    const msg = `Hello Forever Flowers! 🌸\n\nI am interested in the following product:\n\n🌺 *Product:* ${p.name}\n💰 *Price:* ${priceInfo}\n\nIs this available? I would like to place an order.\n\nThank you!`;

    const card = document.createElement("div");
    card.className = "product-card reveal";
    card.dataset.category = p.category || "all";

    // Use <a> tag instead of button+onclick — works perfectly on ALL mobile browsers
    card.innerHTML = `
      <div class="product-img-wrap">
        <img src="${p.image}" alt="${p.name}" loading="lazy" onerror="this.src='https://images.pexels.com/photos/931177/pexels-photo-931177.jpeg?auto=compress&cs=tinysrgb&w=600'"/>
        ${badge}
      ...
      <div class="product-info">
        ...
        <a class="btn-inquire" href="${waLink(msg)}" target="_blank" rel="noopener noreferrer">
...
document.querySelectorAll(".filter-btn").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    const filtered = activeFilter === "all"
      ? products
      : products.filter(p => (p.category || "all") === activeFilter);
...
document.getElementById("builderForm").addEventListener("submit", (e) => {
  ...
  let msg = `Hello Forever Flowers! 🌸\n\nI'd like to build a custom bouquet:\n`;
  msg += `Flowers: ${checked.join(", ")}\n`;
...
document.getElementById("specialForm").addEventListener("submit", (e) => {
  ...
  let msg = `Hello Forever Flowers! 🌸\n\nSpecial Flower Request:\n`;
  if (name)    msg += `Name: ${name}\n`;
  if (flowers) msg += `Flower type/occasion: ${flowers}\n`;
